...
export default function Admin() {
  ...
  async function handleBackfillRanks() {
    setBackfilling(true); setRecalcMsg('')
      ...
      <div className="page-body">
        ...
        {tab==='locks' && roomData && (
          <div style={{display:'flex',flexDirection:'column',gap:'1.25rem'}}>
            ...
            <div className="card" style={{marginBottom:0}}>
              <div className="card-title">Knockout stage predictions</div>
              <p style={{fontSize:13,color:'var(--c-muted)',marginBottom:'0.75rem'}}>
                KO predictions stay locked until you open them. Flip this switch once the bracket is set after the group stage finishes.
              </p>
              <div style={{display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
                <button
                  className={roomData.ko_predictions_open ? 'btn btn-danger' : 'btn btn-accent'}
                  onClick={async function() {
                    var newVal = !roomData.ko_predictions_open
                    await supabase.from('rooms').update({ ko_predictions_open: newVal }).eq('code', adminRoom)
                    setRoomData(Object.assign({}, roomData, { ko_predictions_open: newVal }))
                  }}
                >
                  {roomData.ko_predictions_open ? 'Lock KO predictions' : 'Unlock KO predictions'}
                </button>
                <span style={{display:'flex',alignItems:'center',gap:6}}>
                  ...
                  <span style={{fontSize:12,color:'var(--c-muted)'}}>
                    {roomData.ko_predictions_open ? 'Players can submit KO predictions now.' : 'Players cannot edit KO predictions.'}
                  </span>
                </span>
              </div>
              {roomData.ko_predictions_open && (
